...
sub merge {
    ...
    if ($a ne "") { $returnhash{Action} = "Submit"; }
    if ($a eq "") { $returnhash{Action} = "Modify"; }

	$self->{ERR} = err_messages("verbose");

	$returnhash{ERR} = $self->{ERR}->{msgType} || "";
	$returnhash{Error} = $self->{ERR}->{Error} || "";
	$returnhash{recordid} = $a;
...
sub create {
	...
	# use the err_messages function to get any errors or warnings.
	# return a hash that includes the error type "ERR", the error text "Error" & the record id affected "recordid"
...
sub err_messages {
	# this function looks into $ARS::ars_errhash for error messages and properly parses out Warnings
	# if you pass in the string "error" it just returns the error string
	# if you pass in anything else it returns a hash of information about the most recent error.

	my $ret = shift || "error";

	if ($ARS::ars_errhash{numItems} == 0) { return {}; }
	...
	for my $i (0..$ARS::ars_errhash{numItems}-1) {
		if ($worstMsgType < $ARS::ars_errhash{messageType}->[$i]) { $worstMsgType = $ARS::ars_errhash{messageType}->[$i]; }
		$ARS::ars_errhash{messageType}->[$i] = $errMessageTypes{$ARS::ars_errhash{messageType}->[$i]};
	}

	for my $i (0..$ARS::ars_errhash{numItems}-1) {
		$ARS::ars_errhash{messageText}->[$i] =~ s/^ \(//;
		$ARS::ars_errhash{messageText}->[$i] =~ s/\)$//;
		$ARS::ars_errhash{messageText}->[$i] = q/[/
			. qq/$ARS::ars_errhash{messageType}->[$i] $ARS::ars_errhash{messageNum}->[$i]/
			. q/: ] /
			. $ARS::ars_errhash{messageText}->[$i];
	}


	$newErrorHash{msgText} = $ARS::ars_errhash{messageText};

	$newErrorHash{msgType} = $errMessageTypes{$worstMsgType};

	$newErrorHash{msgNumber} = $ARS::ars_errhash{messageNum};
	$newErrorHash{Error} = join("\n", @{$ARS::ars_errhash{messageText}});
	$newErrorHash{Time} = time();

	if ($ret eq "error") {
		return $newErrorHash{Error};
	} else {
		return \%newErrorHash;
	}
}
